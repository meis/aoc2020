...
module.exports = function(inputFile) {
  ...
  const map = buildMap(tiles, sortedTiles);
  const monsters = findMonsters(map);

  const multiplyBorderIds = Object.values(tiles)
    .filter(t => t.neighbors.length == 2)
    .map(t => t.id)
    .reduce((a, b) => a * b, 1);
  const waterTiles = map.bitmap.flat().flat().filter(c => c == '#').length;

  console.log('The IDs of the four corners multiplied are:', multiplyBorderIds)
  console.log('The water roughness is:', waterTiles - (monsters * 15));
};
...
function buildMap(tiles, sortedTiles) {
  const dimensions = Math.sqrt(tiles.length);
  ...
  return new Tile({ id: 0, bitmap });
}

function findMonsters(tile) {
  let monsters = 0;
  tile.shakeUntil(() => {
    for (let i = 0; i < tile.bitmap.length; i++) {
      for (let j = 0; j < tile.bitmap.length; j++) {
        if (hasMonster(i, j, tile.bitmap)) monsters++;
      }
    }
